...
export default function Invitation({ InvitationList, dataLoading, handleRefresh }) {
  const [appliedApplicationJobId, setAppliedApplicationJobId] = useState(null);
  const [applyJobType, setApplyJobType] = useState(null);
  ...
  const [declineJobData, setDeclineJobData] = useState(null);
  ...
  const getmyapplication = async () => {
    try {
      ...
      if (response?.status == 200) {
        ...
        setApplyJobType(response?.data?.data);
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const handleDeclineInvitation = async (job) => {
    setDeclineJobData(job);
    setSelectedJobId(job?.jobData?._id);
  ...
  const declineInvitation = async () => {
    const dataForNofitication = {
      rout: `/projects/${declineJobData?.jobData?._id}`,
      send_by: "Professional",
      subject: "Decline",
      client_id: declineJobData?.jobData?.user_id?._id,
      job_id: declineJobData?.jobData?._id,
      first_name: declineJobData?.jobData?.user_id?.first_name,
      last_name: declineJobData?.jobData?.user_id?.last_name,
      email: declineJobData?.jobData?.user_id?.email,
    };

    const messageForShow = `${decodedToken.first_name} ${decodedToken.last_name} has declined your invite for ${declineJobData?.jobData?.job_title}`;

    try {
      const data = {
        jobId: selectedJobId, // Use the stored job_id
      };
      await diclinJobInvite(data).then(async (res) => {
        if (res?.status == 200) {
          await postNotification(dataForNofitication).then((res) => {
            ...
            socket.emit("update_notification", { login_as: 1, id: declineJobData?.jobData?.user_id?._id });
          });
          // window.location.reload();
          handleRefresh('invitation')
  ...
  const removedInvitation = async (job_id) => {
    setSubLoader(true);
    try {
      const data = {
        jobId: job_id, // Use the stored job_id
      };
      await removeInvite(data).then((res) => {
        if (res?.status == 200) {
          setSubLoader(false);
          window.location.href = "/invitation&bookmark?removed=true";
        }
      }); // Reload the page or perform any other necessary action
    } catch (error) {
      console.error("Error Removed job invitation:", error);
    }
  };
  return (
    <>
      {dataLoading === true ? (
        ...
        <div className={` ${mulish.className} my-5 pl-[5px]`}>
          {InvitationList.length > 0 ? (
                      ...
                      {item?.jobData.type === "deleted" ? (
                        <>
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              removedInvitation(item?.jobData?._id)
                            }
                          >
                            Remove
                          </button>
            ...
            <div className=" text-black text-center">No data found</div>
          )}
          {validationPopup && (
