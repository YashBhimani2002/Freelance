...
const WorkProgress = (props) => {
  ...
  const calculateTotalMilestonesForJob = (jobId) => {
    const milestonesForJob = allMilestone.filter(milestone => milestone.job_id?._id === jobId);
    ...
    <>
      <div className="main-over-tab">
        <div className="table-responsive overflow-x-auto">
          <table className='table w-full min-w-max' style={{ marginBottom: "16px" }}>
            <thead>
              <tr className={`${inter700.className} font-700`}>
                <th scope="col">PROJECT</th>
                <th scope="col">MILESTONES PROCESS</th>
            ...
            <tbody>
              {Object.keys(groupedMilestones).map(projectId => {
                ...
                const totalMilestonesForJob = calculateTotalMilestonesForJob(projectId);
                // Calculate total amount for the project
                const totalAmountForProject = projectMilestones.reduce((accumulator, milestone) => {
                ...
                // const milestone_process = totalMilestonesForJob - projectMilestones.length;
                ...
                return (
                  lastStatus != 1 ? (
                      ...
                      <tr key={projectId} className='border-t border-[#eee]'>
                        <td className={`p-[12px] text-left text-[#2D3748] text-sm ${inter400.className}`}>
                          {projectMilestones[0].job_id.job_title}
                        </td>
                        <td className={`p-[12px] text-center text-[#2D3748] text-sm ${inter400.className}`}>
                          {`${projectMilestones.length}/${totalMilestonesForJob}`}
                        </td>
                        <td className={`p-[12px] text-center text-[#2D3748] text-sm ${inter400.className}`}>
                          N{fee}
                        </td>
                        <td className={`p-[12px] text-right text-[#2D3748] text-sm ${inter400.className}`}>
          ...
          <table className='table table-total' style={{ marginTop: "40px" }}>
            <thead>
              <tr className={`${inter700.className} font-700`}>
                ...
                <th scope="col">N{calculateTotalAmount(milestonePayment)}</th>
              </tr>
            </thead>
            <tbody></tbody>
          </table>
